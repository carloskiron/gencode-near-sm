...
const main = () => {
  ...
  try {
    ...
    model.filter(({onChain}) => onChain).forEach((entity) => {
      ejs.renderFile(filename, {entity}, options, function (err, str) {
        // str => Rendered HTML string
        if (err) {
          console.error(err);
        }

        const outputFile = path.join(process.cwd(), `${entity.name}.ts`);
